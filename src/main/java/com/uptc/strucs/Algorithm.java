...
public class Algorithm extends Automaton {
    ...
    private void makeAutomaton() {
        table.stream().filter(x -> !x.isCheck).forEach(pair -> {
            ...
            if(!pair.a.isCombinated && !pair.b.isCombinated){
                ...
                if(pair.a.isCombinated && pair.b.isCombinated) return;
    ...
    private void addTransitionsOutput(State newState, State oldState) {
        graph.stream().filter(x -> x.equals(oldState)).findFirst().ifPresent(x -> 
            x.transitions.forEach(y -> 
                newState.addTransition(y.state.equals(oldState) ? newState : y.state, y.terminalSymbol)
            )
        );
    }
    ...
    private void addTransitionsInput(State newState, State oldState) {
        graph.forEach(state -> 
            state.transitions.stream()
                        .filter(x -> x.state.equals(oldState))
